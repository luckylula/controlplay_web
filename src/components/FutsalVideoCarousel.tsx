...
const FUTSAL_VIDEOS = [
  "/images/futsal/futsal%20video1.mp4",
  "/images/futsal/futsal%20video2.mp4",
  "/images/futsal/futsal%20video3.mp4",
];

const INTERVAL_MS = 6000;

export function FutsalVideoCarousel() {
  const [index, setIndex] = useState(0);

  useEffect(() => {
    const id = setInterval(() => {
      setIndex((i) => (i + 1) % FUTSAL_VIDEOS.length);
    }, INTERVAL_MS);
      ...
      {FUTSAL_VIDEOS.map((src, i) => (
        ...
        >
          <video
            ...
            style={{ objectPosition: i === 2 ? "center 70%" : "center center" }}
            src={src}
          />
        </div>
      ))}
