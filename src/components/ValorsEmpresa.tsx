    ...
    >
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        {valors.map((valor) => (
          <article
            key={valor.title}
            className="relative overflow-visible rounded-3xl bg-blue-900 p-8 pb-10 pt-16 shadow-xl sm:p-10 sm:pt-20"
          >
            {/* Imagen flotante en la esquina superior */}
            <div className="absolute left-6 top-0 z-10 -translate-y-1/2 sm:left-8">
              {valor.image ? (
                ...
                <div
                  className="flex h-20 w-20 items-center justify-center rounded-2xl border-4 border-white bg-blue-700 shadow-lg sm:h-24 sm:w-24"
                  aria-hidden
                >
